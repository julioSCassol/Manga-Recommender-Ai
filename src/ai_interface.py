...
class AISession:
    ...
    def start_session(self):
        ...
        if initial_data and isinstance(initial_data, list):
            ...
            for i, manga_summary in enumerate(initial_data):
                ...
                if manga_id:
                    ...
                    if full_manga_details:
                        ...
                        print(f"Failed to fetch full details for manga ID: {manga_id}")
                else:
            ...
            if processed_data:
                ...
                print(f"Successfully initialized recommender with {len(processed_data)} manga entries.")
            else:
                raise ValueError("No valid manga data processed after fetching full details.")
        ...
        for tag in tags:
            if isinstance(tag, dict) and tag.get('group') == group:
                ...
                if isinstance(name, dict):
                    result.append(name.get('en') or next(iter(name.values()), ''))
        return result
    ...
    def get_recommendations(self):
        ...
        preferences = self.user_profile.get('preferences') or self.user_profile.get('initial_prefs')
        ...
        if preferences:
            ...
            for idx, manga in enumerate(data):
                ...
                if 'authors' in preferences:
                    for author in manga.get('authors', []):
                        if author in preferences['authors']:
                            author_count = preferences['authors'][author] if isinstance(preferences['authors'], dict) else 1
                ...
                if 'artists' in preferences:
                    for artist in manga.get('artists', []):
                        if artist in preferences['artists']:
                            artist_count = preferences['artists'][artist] if isinstance(preferences['artists'], dict) else 1
                ...
                if 'avg_rating' in preferences and preferences['avg_rating'] and manga.get('rating'):
                    rating_diff = abs(manga['rating'] - preferences['avg_rating'])
    ...
    def get_similarity_reason(self, manga_idx):
        ...
        common_genres = set(manga.get('genres', [])) & set(prefs['genres'].keys())
        if common_genres:
            reasons.append(f"Gêneros similares: {', '.join(common_genres)}")

        common_themes = set(manga.get('themes', [])) & set(prefs['themes'].keys())
        if common_themes:
            reasons.append(f"Temas similares: {', '.join(common_themes)}")

        common_authors = set(manga.get('authors', [])) & set(prefs['authors'].keys())
        if common_authors:
